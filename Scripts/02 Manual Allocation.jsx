...
/**************** PURPOSE ****************
    ...
    before running this script to ensure that it will run as expected. This script will offer options in terms of allocation: if single side or double side.
...
/**
    * status - inprogress
    * requests if the current selected boundaries are to be assigned on one side only or both sides
    * @return   1 if single sided, 2 if double sided, 0 if cancelled
    */
function chooseSide(){
        ...
        //create a window
        var sides = new Window("dialog", 'Sides');
        
        var inputGroup = sides.add("group");
        inputGroup.alignment = "centre";
        inputGroup.add("statictext", undefined, 'Select the number of widths to allocate capstone groups along.');
        
        var radioButtons = sides.add("Group");
        var singleSide = radioButtons.add("radiobutton", [15,15,130,30], 'Single side');
        var doubleSide = radioButtons.add("radiobutton", [15,15,130,30], 'Double side');
        //default button is double sided
        doubleSide.value = true;
        
        var buttons = sides.add("group");
        buttons.alignment = "right";
        var buttonSubmit = buttons.add("button", undefined, 'OK');
        var buttonCancel = buttons.add("button", undefined, 'Cancel');
...
/**
    * status - inprogress
    * takes in the completed array of information and run the algorithm for single side selection
    * @param     myDoc   current document being worked on
    * @param    array   the completed array of information
    */
function singleAllocation(myDoc, array, scale){
        ...
        //after assigning, draw out the groups that have not been assigned
        for(var k = 0; k < capstone.length; k++){
            
            if (capstone[k]["assigned"] == false) {
...
/**
    * status - completed
    * takes in the completed array of information and run the algorithm for double side selection
    * @param    myDoc   current document being worked on
    * @param    array   the completed array of information
    */
function doubleAllocation(myDoc, array, scale){
        ...
        // convert capstone values to be to scale
        for(var i = 0; i < capstone.length; i++){
            $.writeln("Showcase length in millimeters: ", capstone[i]["showcase length"]);
            capstone[i]["lengthInPoints"] = capstone[i]["showcase length"] /(scale * 0.3528);
            capstone[i]["widthInPoints"] = capstone[i]["showcase width"] /(scale * 0.3528);
            $.writeln("Showcase length in points: ", capstone[i]["lengthInPoints"]);
            }
...
function main(){
    ...
    while(1){
        ...
        //select the number of sides to assign along
        var sides = chooseSide();
        if(sides === 0){
            //if cancel, close script
            terminatingScript ();
            break;
            }
        else if(sides === 1){
            singleAllocation (banana, initialArray, scale);
            }
        else if(sides === 2){
            doubleAllocation(banana, initialArray, scale);
            }
